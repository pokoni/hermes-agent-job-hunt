...
def test_submission_review_has_pdf_export_section() -> None:
    b = _basename()
...
def test_submission_decision_links_pdf_export_artifacts() -> None:
    b = _basename()
    export_manifest = _read_json(f"outputs/resumes/{b}_pdf_export_manifest.json")
    ...
    generated = {item["document_type"]: item["output_pdf"] for item in export_manifest["generated_files"]}
    assert decision["resume_pdf_file"] == generated["resume_ja"]
    assert decision["cv_pdf_file"] == generated["cv_ja"]
    assert decision["pdf_export_manifest"] == f"outputs/resumes/{b}_pdf_export_manifest.json"
    assert decision["pdf_human_visual_review_required"] is True
    assert decision["human_review_required"] is True
    assert decision["explicit_human_approval_required"] is True


def test_submission_review_does_not_report_pdf_missing_when_export_exists() -> None:
    b = _basename()
    decision = _read_json(f"outputs/logs/{b}_submission_decision.json")
    ...
    assert not found, f"Review still reports PDF artifacts as missing: {found}"


def test_submission_review_keeps_human_approval_boundary_with_pdf() -> None:
    b = _basename()
    text = _read_text(f"outputs/logs/{b}_submission_review.md").lower()
    required = [
