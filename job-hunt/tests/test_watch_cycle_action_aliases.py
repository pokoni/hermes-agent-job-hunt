...
for attr in [
    'output','batch_output','ranking_json','ranking_md','queue_jsonl',
    'output_jsonl','report','delivery_log','alias_map'
]:
...
def test_watch_cycle_uses_action_aliases_by_default(tmp_path: Path) -> None:
    ...
    assert report["telegram_action_aliases_enabled"] is True

    render_step = next(step for step in report["steps"] if step["name"] == "render_telegram_job_notifications")
    assert "--alias-map" in render_step["command"]
    assert "outputs/logs/telegram_action_alias_map.json" in render_step["command"]
    assert "--use-action-aliases" in render_step["command"]


def test_watch_cycle_can_disable_action_aliases(tmp_path: Path) -> None:
    _install_stubs(tmp_path)
    output = tmp_path / "outputs" / "logs" / "job_watch_cycle_report.json"
    ...
    report = json.loads(output.read_text(encoding="utf-8"))
    assert report["status"] == "passed"
    assert report["telegram_action_aliases_enabled"] is False

    render_step = next(step for step in report["steps"] if step["name"] == "render_telegram_job_notifications")
    assert "--alias-map" in render_step["command"]
    assert "--use-action-aliases" not in render_step["command"]


def test_watch_cycle_preserves_dry_run_send_boundary(tmp_path: Path) -> None:
    _install_stubs(tmp_path)
    output = tmp_path / "outputs" / "logs" / "job_watch_cycle_report.json"
    ...
    assert "--send" not in send_step["command"]
