...
def build_fixture_artifacts(
    ...
    for idx, row in enumerate(candidates, start=1):
        ...
        alias_entries.append({
            ...
            "commands": {
                "generate": f"/{action_prefix}_generate {alias}",
                "track": f"/{action_prefix}_track {alias}",
                "ignore": f"/{action_prefix}_ignore {alias}",
                "defer": f"/{action_prefix}_defer {alias}",
            },
            "resolved_commands": {
                "generate": f"/{action_prefix}_generate {action_id}",
                "track": f"/{action_prefix}_track {action_id}",
                "ignore": f"/{action_prefix}_ignore {action_id}",
                "defer": f"/{action_prefix}_defer {action_id}",
            },
        })
    ...
    for idx, row in enumerate(candidates, start=1):
        digest_lines += [
            ...
            f"Generate: /{action_prefix}_generate {idx}",
            f"Track: /{action_prefix}_track {idx}",
            f"Ignore: /{action_prefix}_ignore {idx}",
        ]
