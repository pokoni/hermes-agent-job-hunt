...
DEFAULT_DUPLICATE_DISPLAY_LIMIT = 50
...
def sort_key(row: dict) -> tuple[bool, bool, int, bool]:
    decision = row.get("computed_ranking_decision") or row.get("ranking_decision", "")
    return (
        decision == "suggest_generate_materials_after_user_approval",
...
def score_items(
    ...
    for item in items:
        ...
        computed_decision = decision_for(
            score["fit_score"],
            source,
            defaults,
            score["topic_quality_label"],
        )
        decision = "already_seen_display_only" if display_only else computed_decision

        row = {
            **normalized_job,
            **score,
            "ranking_decision": decision,
            "computed_ranking_decision": computed_decision,
            "discovery_status": discovery_status,
            "duplicate_reason": item.get("duplicate_reason", ""),
            "notification_suppressed_reason": "job_fingerprint_already_seen" if display_only else "",
...
def build_reports(
    ...
    sources: dict,
    profile: dict,
    duplicate_display_limit: int = DEFAULT_DUPLICATE_DISPLAY_LIMIT,
) -> dict:
    src_map = source_by_id(sources)
    defaults = sources.get("default_thresholds", {})

    new_candidates = score_items(
        workspace=workspace,
        items=dedup_report.get("new_jobs", []),
        src_map=src_map,
        defaults=defaults,
        profile=profile,
    ...
    duplicate_display_candidates: list[dict] = []
    if not new_candidates and duplicate_display_limit != 0:
        duplicate_display_candidates = score_items(
            workspace=workspace,
            items=dedup_report.get("duplicates", []),
            src_map=src_map,
            defaults=defaults,
            profile=profile,
        ...
        if duplicate_display_limit > 0:
            duplicate_display_candidates = duplicate_display_candidates[:duplicate_display_limit]

    candidates = new_candidates or duplicate_display_candidates

    notify = [row for row in new_candidates if row["ranking_decision"] in {
        "notify_user",
        "suggest_generate_materials_after_user_approval",
    }]
    material = [row for row in new_candidates if row["ranking_decision"] == "suggest_generate_materials_after_user_approval"]
    hold = [row for row in new_candidates if row["ranking_decision"] == "hold"]
    ...
    return {
        ...
        "display_duplicate_candidate_count": len(duplicate_display_candidates),
        "display_duplicate_limit": duplicate_display_limit,
...
def markdown_report(report: dict) -> str:
    lines = [
        ...
        f"- New candidate count: `{report.get('new_candidate_count', report['candidate_count'])}`",
        f"- Duplicate display count: `{report.get('display_duplicate_candidate_count', 0)}`",
    ...
    lines += [
        ...
        "- Duplicate-seen jobs may be scored for display only; they are not re-notified.",
...
def ranking_payload(report: dict) -> dict:
    return {
        "status": report["status"],
        "run_at": report["run_at"],
        "candidate_count": report["candidate_count"],
        "new_candidate_count": report.get("new_candidate_count", report["candidate_count"]),
        "display_duplicate_candidate_count": report.get("display_duplicate_candidate_count", 0),
        "notify_count": report["notify_count"],
        "material_suggestion_count": report["material_suggestion_count"],
...
def main() -> int:
    ...
    parser.add_argument("--last-nonempty-ranking-json", default="outputs/logs/job_ranking_gate_decision_last_nonempty.json")
    ...
    parser.add_argument("--duplicate-display-limit", type=int, default=DEFAULT_DUPLICATE_DISPLAY_LIMIT, help="Score this many duplicate-seen jobs for display only when no new jobs exist; -1 means all, 0 disables.")
    ...
    report = build_reports(
        ...
        sources,
        profile,
        duplicate_display_limit=args.duplicate_display_limit,
    )

    batch_output = resolve(args.batch_output)
    ranking_json = resolve(args.ranking_json)
    last_nonempty_ranking_json = resolve(args.last_nonempty_ranking_json)
    ...
    payload = ranking_payload(report)
    ranking_json.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    if report["candidate_count"] > 0:
        last_nonempty_ranking_json.parent.mkdir(parents=True, exist_ok=True)
        snapshot = {
            **payload,
            "snapshot_type": "last_nonempty_ranking",
            "snapshot_updated_at": now_iso(),
        }
        last_nonempty_ranking_json.write_text(json.dumps(snapshot, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
