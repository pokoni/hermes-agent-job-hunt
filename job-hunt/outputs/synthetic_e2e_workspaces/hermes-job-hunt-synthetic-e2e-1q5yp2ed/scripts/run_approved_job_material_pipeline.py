...
def run_material_pipeline(
    ...
    pipeline_stages = [item["stage"] for item in commands]
    ...
    commands_doc = {
        ...
        "job_basename": job_basename,
        "layer_contract": {
            "layer1_normalized_job": f"data/jobs/{job_basename}.json",
            "layer2_inputs": [
                f"data/jobs/{job_basename}.json",
                "data/candidate_profile.json",
            ],
            "layer2_only": not include_normalizer,
        },
...
def main() -> int:
    ...
    parser.add_argument("--include-normalizer", action="store_true")
    parser.add_argument(
        "--layer2-only",
        action="store_true",
        help="Expect Layer1 to have already written data/jobs/<basename>.json.",
    )
    ...
    report = run_material_pipeline(
        ...
        include_normalizer=args.include_normalizer and not args.layer2_only,
    )
