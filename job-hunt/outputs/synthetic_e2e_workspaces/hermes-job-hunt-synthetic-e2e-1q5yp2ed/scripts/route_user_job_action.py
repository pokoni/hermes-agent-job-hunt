...
This script converts a user action such as:
  ...
  /job_generate <action_id>
  /job_track <action_id>
  /job_ignore <action_id>
  /job_defer <action_id>
  /job_review <action_id>

or the legacy compact form:
...
ACTION_PATTERNS = [
    re.compile(r"^/?(?P<prefix>[a-zA-Z0-9_-]+)[_-](?P<action>generate|track|ignore|defer|review)\s+(?P<action_id>[a-zA-Z0-9_-]+)$"),
    re.compile(r"^/?(?P<prefix>[a-zA-Z0-9_-]+)_(?P<action>generate|track|ignore|defer|review)_(?P<action_id>[a-zA-Z0-9_-]+)$"),
]
...
def route_action(
    ...
    result_path: Path,
    alias_map_path: Path,
    fallback_alias_map_path: Path,
    fallback_ranking_path: Path,
    note: str,
) -> dict:
    try:
    ...
    if not alias_map.get("aliases"):
        alias_map = load_json(fallback_alias_map_path)
    ...
    fallback_ranking = load_json(fallback_ranking_path)
    ...
    if not candidate:
        candidate = find_candidate_from_ranking(fallback_ranking, notification, action_id)
...
def main() -> int:
    ...
    parser.add_argument("--fallback-alias-map", default="outputs/logs/telegram_action_alias_map_last_nonempty.json")
    parser.add_argument("--fallback-ranking", default="outputs/logs/job_ranking_gate_decision_last_nonempty.json")
    ...
    result = route_action(
        ...
        fallback_alias_map_path=resolve(args.fallback_alias_map),
        fallback_ranking_path=resolve(args.fallback_ranking),
        note=args.note,
    )
