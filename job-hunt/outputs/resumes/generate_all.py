#!/usr/bin/env python3
"""One-step generator: reads Markdown -> writes DOCX + PDF + manifests.

Usage:
    python3 outputs/resumes/generate_all.py

Or from workspace root:
    python3 outputs/resumes/generate_all.py
"""

import base64, json, zipfile, io, re
...
def main():
    ...
    for dt in ["resume_ja", "cv_ja"]:
        ...
        dp.write_bytes(db)
        print(f"DOCX created: {dp} ({len(db)} bytes)")

        pb = make_fallback_pdf(db, dt)
        pp = RESUME_DIR / f"{BASENAME}_{dt}.pdf"
        pp.write_bytes(pb)
        print(f"PDF created: {pp} ({len(pb)} bytes)")
