...
def load_runtime_env() -> list[str]:
    """Load Hermes .env so detached subprocesses can send Telegram packages."""
    try:
        from hermes_cli.env_loader import load_hermes_dotenv

        return [str(path) for path in load_hermes_dotenv(project_env=Path(__file__).resolve().parents[2] / ".env")]
    except Exception:
        return []
...
def main() -> int:
    ...
    loaded_env_paths = load_runtime_env()
    ...
    report["loaded_env_paths"] = loaded_env_paths
